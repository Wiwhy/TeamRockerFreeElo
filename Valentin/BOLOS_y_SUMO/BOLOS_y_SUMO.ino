...
const int  VEL_INICIO_RETRO     = 80;  // PWM marcha atras en maniobra inicio
const int  VEL_INICIO_AVANCE    = 200;  // PWM avance para volver al centro
const int  TIEMPO_INICIO_AVANCE = 530;  // ms avanzando hacia el centro tras detectar linea trasera

// --- Velocidades de movimiento ---
const int VEL_BUSQUEDA          = 160;  // PWM giro de busqueda          (0-255)
const int VEL_ATAQUE            = 200;  // PWM avance de ataque          (0-255)
const int VEL_RETROCESO         = 190;  // PWM marcha atras al centro    (0-255)
  ...
  switch (estado) {
    ...
    case RETROCEDER:
      ...
      if (millis() - tRetroIni < (unsigned long)TIEMPO_RETROCESO) {
        ...
        delay(DELAY_POST_RETRO); // Espera a que la inercia del robot se estabilice

        // ---> NUEVO: Limpieza de eco fantasma <---
        // Hacemos una lectura que se descarta automáticamente. 
        // Esto asegura que el sensor HC-SR04 esté vacío y no detecte 
        // rebotes del suelo causados por la frenada antes de buscar.
        medirMediana(); 
