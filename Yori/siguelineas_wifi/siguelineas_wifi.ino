...
// ⚠️ MODO DEL ROBOT (0 = Frenado, 1 = Corriendo, 2 = Girando Derecha)
int modo_robot = 0;        
unsigned long ultimo_wifi = 0;   

#define MAX_SPEED  255
#define BASE_SPEED 210   // Velocidad en rectas perfectas
...
void loop() {
  ...
  if (modo_robot == 1) {
    ...
    setMotors(120, -120); // Gira a la derecha sobre sí mismo a velocidad 120
  } 
  else {
    stop_bot();           
  }
    ...
    if (len > 0) {
      ...
      if (packetBuffer[0] == 'F') {
        ...
        if (token != NULL) {
          ...
          Serial.println("¡Cerebro Fuzzy Actualizado!");
        }
      } 
